	...
	window.addEventListener('DOMContentLoaded', (event) => {
		const canvas = document.querySelector('canvas#storage-request');
			...
			afterDatasetDraw(chart) {
				...
				// ✅ Define labels properly (0–100 scale)
				const labels = [];
				const min = 0;
				const max = 100;
				const steps = 5; // number of labels (0,25,50,75,100)

				for (let i = 0; i <= steps; i++) {
				const value = min + (i * (max - min) / steps);
				const angle = Math.PI + (i / steps) * Math.PI; // half circle
				labels.push({
					text: value.toString(),
					angle: angle
				});
				...
				labels.forEach(l => {
				const x = cx + Math.cos(l.angle) * labelRadius;
				const y = cy + Math.sin(l.angle) * labelRadius;
				ctx.fillText(l.text, x, y);
				});

				// 4. Draw Needle
				const needleValue = 55.6; // Your data value
		...
		if (canvas && typeof canvas.getContext === 'function') {
			...
			const percentage = 60; // The desired percentage value
			...
			const colors = data.map((_, i) =>
				i < filledSegments ? '#E41F07' : '#F3F4F6'
			);
