$(document).ready(function () {
    if($('#device-management').length > 0) {
		$('#device-management').DataTable({
				"bFilter": false, 
				"bInfo": false,
					"ordering": true,
				"autoWidth": true,
				"language": {
					search: ' ',
					sLengthMenu: '_MENU_',
					searchPlaceholder: "Search",
					info: "_START_ - _END_ of _TOTAL_ items",
					"lengthMenu":     "Show _MENU_ entries",
					paginate: {
					next: '<i class="ti ti-chevron-right"></i> ',
					previous: '<i class="ti ti-chevron-left"></i> '
				},
					},
				initComplete: (settings, json)=>{
					$('.dataTables_paginate').appendTo('.datatable-paginate');
					$('.dataTables_length').appendTo('.datatable-length');
				},  
				"data":[
					{
						"id" : "#DV0020",
                        "login-time" : "29 Jan 2026, 08:00 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "Windows 11",
						"si_no" : "",
						"star" : "",
                        "members" : "18 Members",
                        "module" : "Chrome",
                        "name" : "Update",
						"date" : "Dec 2026",
                        "record-id" : "DL0020",
						"client" : "Elijah Blackwood",
						"pro_img" : "assets/img/priority/truellysel.svg",
						"client_img": "assets/img/icons/company-icon-01.svg",
						"piority" : "0",
						"start_date" : "25 Sep 2025",
						"end_date" : "60%",
						"stage" : "0",
						"type" : "Desktop",
						"status" : "0",
						"value": "$250,000",
						"hrs": "$250,000",
						"mem_image1" : "assets/img/profiles/avatar-14.jpg",
						"mem_image2": "assets/img/profiles/avatar-15.jpg",
						"mem_image3": "assets/img/profiles/avatar-16.jpg",
                        "location_image" : "assets/img/flags/us.svg",
                        "location_name" : "USA",
						"budget": "$200000",
						"currently_spend": "$40000",
                        "ip": "192.168.1.12",
						"Action" : ""
					},
					{
						"id" : "#DV0019",
                        "login-time" : "29 Jan 2026, 09:15 AM",
                        "logout-time" : "29 Jan 2026, 09:15 PM",
                        "device" : "macOS 26",
						"si_no" : "",
						"star" : "",
                        "name" : "Create",
                        "members" : "20 Members",
                        "module" : "Safari",
						"date" : "Nov 2026",
						"client" : "Scarlett Beaumont",
                        "record-id" : "LE0016",
						"pro_img" : "assets/img/priority/dreamchat.svg",
						"client_img": "assets/img/icons/company-icon-02.svg",
						"piority" : "0",
						"start_date" : "29 Sep 2025",
						"end_date" : "0%",
						"stage" : "1",
						"type" : "Desktop",
						"status" : "2",
						"value": "$50,000",
						"hrs": "80",
						"mem_image1" : "assets/img/profiles/avatar-03.jpg",
						"mem_image2": "assets/img/profiles/avatar-05.jpg",
						"mem_image3": "assets/img/profiles/avatar-06.jpg",
                         "location_image" : "assets/img/flags/ae.svg",
                        "location_name" : "UAE",
						"budget": "$300000",
						"currently_spend": "$120000",
                        "ip": "192.168.1.42",
						"Action" : ""
					},
					{
						"id" : "#DV0018",
                        "login-time" : "29 Jan 2026, 08:30 AM",
                        "logout-time" : "29 Jan 2026, 08:30 PM",
                        "device" : "Android 16",
						"si_no" : "",
						"star" : "",
                        "name" : "Delete",
                        "members" : "32 Members",
                        "module" : "CRM App",
						"date" : "Oct 2026",
						"client" : "Owen Sterling",
                        "record-id" : "CN0027",
						"pro_img" : "assets/img/priority/truellysell.svg",
						"client_img": "assets/img/icons/company-icon-03.svg",
						"piority" : "0",
						"start_date" : "05 Oct 2025",
						"end_date" : "50%",
						"stage" : "1",
						"type" : "Mobile",
						"status" : "1",
						"value": "$45,000",
						"hrs": "$40,000",
						"mem_image1" : "assets/img/profiles/avatar-04.jpg",
						"mem_image2": "assets/img/profiles/avatar-01.jpg",
						"mem_image3": "assets/img/profiles/avatar-16.jpg",
                        "location_image" : "assets/img/flags/de.svg",
                        "location_name" : "Germany",
						"budget": "$200000",
						"currently_spend": "$200000",
                        "ip": "192.168.1.12",
						"Action" : ""
					},
					{
						"id" : "#DV0017",
                        "login-time" : "29 Jan 2026, 09:00 AM",
                        "logout-time" : "29 Jan 2026, 09:00 PM",
                        "device" : "Windows 11",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "40 Members",
                        "module" : "Edge",
						"date" : "Sep 2026",
						"client" : "Hazel Davenport",
                        "record-id" : "PR0039",
						"pro_img" : "assets/img/priority/servbook.svg",
						"client_img": "assets/img/icons/company-icon-04.svg",
						"piority" : "0",
						"start_date" : "14 Oct 2025",
						"end_date" : "80%",
						"stage" : "1",
						"type" : "Mobile",
						"status" : "1",
						"value": "$780,000",
						"hrs": "$45,000",
						"mem_image1" : "assets/img/profiles/avatar-12.jpg",
						"mem_image2": "assets/img/profiles/avatar-15.jpg",
						"mem_image3": "assets/img/profiles/avatar-13.jpg",
                        "location_image" : "assets/img/flags/fr.svg",
                        "location_name" : "France",
						"budget": "$300000",
						"currently_spend": "$60000",
                        "ip": "192.168.1.48",
						"Action" : ""
					},
					{
						"id" : "#DV0016",
                        "login-time" : "29 Jan 2026, 09:20 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "iOS 26",
						"si_no" : "",
						"star" : "",
                        "name" : "Export",
                        "members" : "58 Members",
                        "module" : "Edge",
						"date" : "Aug 2026",
						"client" : "Violet Ainsworth",
                        "record-id" : "RE0042",
						"pro_img" : "assets/img/priority/dream-pos.svg",
						"client_img": "assets/img/icons/company-icon-05.svg",
						"piority" : "0",
						"start_date" : "15 Nov 2025",
						"end_date" : "100%",
						"stage" : "0",
						"type" : "Mobile",
						"status" : "0",
						"value": "$80,000",
						"hrs": "$85,000",
						"mem_image1" : "assets/img/profiles/avatar-10.jpg",
						"mem_image2": "assets/img/profiles/avatar-11.jpg",
						"mem_image3": "assets/img/profiles/avatar-16.jpg",
                        "location_image" : "assets/img/flags/india.svg",
                        "location_name" : "India",
						"budget": "$120000",
						"currently_spend": "$40000",
                        "ip": "192.168.1.45",
						"Action" : ""
					},
					{
						"id" : "#DV0015",
                        "login-time" : "29 Jan 2026, 09:30 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "Windows 11",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "27 Members",
                        "module" : "Firefox",
						"date" : "Jul 2026",
						"client" : "Milo Rutherford",
                        "record-id" : "CM0051",
						"pro_img" : "assets/img/priority/project-01.svg",
						"client_img": "assets/img/icons/company-icon-06.svg",
						"piority" : "0",
						"start_date" : "25 Nov 2025",
						"end_date" : "60%",
						"stage" : "1",
						"type" : "Desktop",
						"status" : "2",
						"value": "$40,000",
						"hrs": "$180,000",
						"mem_image1" : "assets/img/profiles/avatar-14.jpg",
						"mem_image2": "assets/img/profiles/avatar-09.jpg",
						"mem_image3": "assets/img/profiles/avatar-08.jpg",
                        "location_image" : "assets/img/flags/brazil.svg",
                        "location_name" : "Brazil",
						"budget": "$200000",
						"currently_spend": "$90000",
                        "ip": "192.168.1.26",
						"Action" : ""
					},
					{
						"id" : "#DV0014",
                        "login-time" : "29 Jan 2026, 08:50 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "Android 16",
						"si_no" : "",
						"star" : "",
                        "name" : "Create",
                        "members" : "40 Members",
                        "module" : "Firefox",
						"date" : "Jun 2026",
						"client" : "Luna Ashworth",
                        "record-id" : "TS0023",
						"pro_img" : "assets/img/priority/project-02.svg",
						"client_img": "assets/img/icons/company-icon-07.svg",
						"piority" : "0",
						"start_date" : "08 Dec 2025",
						"end_date" : "100%",
						"stage" : "0",
						"type" : "Desktop",
						"status" : "0",
						"value": "$7,000",
						"hrs": "$90,000",
						"mem_image1" : "assets/img/profiles/avatar-06.jpg",
						"mem_image2": "assets/img/profiles/avatar-07.jpg",
						"mem_image3": "assets/img/profiles/avatar-16.jpg",
                        "location_image" : "assets/img/flags/mexico.svg",
                        "location_name" : "Mexico",
						"budget": "$200000",
						"currently_spend": "$195000",
                        "ip": "192.168.1.50",
						"Action" : ""
					},
					{
						"id" : "#DV0013",
                        "login-time" : "29 Jan 2026, 09:12 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "macOS 26",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "18 Members",
                        "module" : "Safari",
						"date" : "Apr 2026",
						"client" : "Scarlett Beaumont",
                        "record-id" : "DL0017",
						"pro_img" : "assets/img/priority/best.svg",
						"client_img": "assets/img/icons/company-icon-08.svg",
						"piority" : "0",
						"start_date" : "21 Dec 2025",
						"end_date" : "10%",
						"stage" : "1",
						"type" : "Desktop",
						"status" : "2",
						"value": "$01,23,000",
						"hrs": "$80,000",
						"mem_image1" : "assets/img/profiles/avatar-04.jpg",
						"mem_image2": "assets/img/profiles/avatar-15.jpg",
						"mem_image3": "assets/img/profiles/avatar-05.jpg",
                        "location_image" : "assets/img/flags/china.svg",
                        "location_name" : "China",
						"budget": "$45,000",
						"currently_spend": "$220000",
                        "ip": "192.168.1.30",
						"Action" : ""
					},
					{
						"id" : "#DV0012",
                        "login-time" : "29 Jan 2026, 08:55 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "iOS 26",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "60 Members",
                        "module" : "Safari",
						"date" : "Apr 2026",
						"client" : "Jasper Huntington",
                        "record-id" : "IN0010",
						"pro_img" : "assets/img/priority/dream-pos.svg",
						"client_img": "assets/img/icons/company-icon-06.svg",
						"piority" : "0",
						"start_date" : "01 Jan 2024",
						"end_date" : "100%",
						"stage" : "0",
						"type" : "Mobile",
						"status" : "0",
						"value": "$780,000",
						"hrs": "$1180,000",
						"mem_image1" : "assets/img/profiles/avatar-08.jpg",
						"mem_image2": "assets/img/profiles/avatar-12.jpg",
						"mem_image3": "assets/img/profiles/avatar-04.jpg",
                        "location_image" : "assets/img/flags/russia.svg",
                        "location_name" : "Russia",
						"budget": "$200000",
						"currently_spend": "$177777 ",
                        "ip": "192.168.1.22",
						"Action" : ""
					},
					{
						"id" : "#DV0011",
                        "login-time" : "29 Jan 2026, 09:05 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "iOS 26",
						"si_no" : "",
						"star" : "",
                        "name" : "Create",
                        "members" : "18 Members",
                        "module" : "Safari",
						"date" : "Jan 2026",
						"client" : "Caleb Worthington",
                        "record-id" : "PS0018",
						"pro_img" : "assets/img/priority/servbook.svg",
						"client_img": "assets/img/icons/company-icon-09.svg",
						"piority" : "0",
						"start_date" : "12 Jan 2024",
						"end_date" : "50%",
						"stage" : "0",
						"type" : "Mobile",
						"status" : "0",
						"value": " $04,10,000",
						"hrs": "$14,10,000",
						"mem_image1" : "assets/img/profiles/avatar-12.jpg",
						"mem_image2": "assets/img/profiles/avatar-14.jpg",
						"mem_image3": "assets/img/profiles/avatar-01.jpg",
                        "location_image" : "assets/img/flags/fr.svg",
                        "location_name" : "France",
						"budget": "$80,000",
						"currently_spend": "$100000 ",
                        "ip": "192.168.1.18",
						"Action" : ""
					},
					{
						"id" : "#DV0010",
                        "login-time" : "29 Jan 2026, 08:00 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "Windows 11",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "28 Members",
                        "module" : "Firefox",
						"date" : "Apr 2026",
						"client" : "Elijah Blackwood",
                        "record-id" : "PS0018",
						"pro_img" : "assets/img/priority/sports.svg",
						"client_img": "assets/img/icons/company-icon-09.svg",
						"piority" : "0",
						"start_date" : "16 Jan 2024",
						"end_date" : "60%",
						"stage" : "0",
						"type" : "Mobile",
						"status" : "0",
						"value": "$02,19,000",
						"hrs": "04,10,000",
						"mem_image1" : "assets/img/profiles/avatar-08.jpg",
						"mem_image2": "assets/img/profiles/avatar-15.jpg",
						"mem_image3": "assets/img/profiles/avatar-12.jpg",
                        "location_image" : "assets/img/flags/italy.svg",
                        "location_name" : "Italy",
						"budget": "$780,000",
						"currently_spend": "$200000 ",
                        "ip": "192.168.1.15",
						"Action" : ""
					},
					{
						"id" : "#DV0009",
                        "login-time" : "29 Jan 2026, 09:05 AM",
                        "logout-time" : "29 Jan 2026, 08:00 PM",
                        "device" : "Windows 11",
						"si_no" : "",
						"star" : "",
                        "name" : "Update",
                        "members" : "58 Members",
                        "module" : "Firefox",
						"date" : "Oct 2026",
						"client" : "Aurora Lancaster",
                        "record-id" : "PS0018",
						"pro_img" : "assets/img/priority/best.svg",
						"client_img": "assets/img/icons/company-icon-08.svg",
						"piority" : "0",
						"start_date" : "12 Jan 2024",
						"end_date" : "60%",
						"stage" : "0",
						"type" : "Mobile",
						"status" : "0",
						"value": "$60,000",
						"hrs": "4,10,000",
						"mem_image1" : "assets/img/profiles/avatar-01.jpg",
						"mem_image2": "assets/img/profiles/avatar-11.jpg",
						"mem_image3": "assets/img/profiles/avatar-14.jpg",
                        "location_image" : "assets/img/flags/canada.svg",
                        "location_name" : "Canada",
						"budget": "$300000",
						"currently_spend": "$100000 ",
                        "ip": "192.168.1.12",
						"Action" : ""
					}
				],
			"columns": [
                { "render": function ( data, type, row ){ return '<a href="#">'+row['id']+'</a>'; }},
                { "render": function ( data, type, row ){
					return '<h6 class="d-flex align-items-center fs-14 mb-0 fw-medium"><a href="#" class="avatar avatar-sm border rounded-circle me-2"><img class="rounded-circle" src="'+row['mem_image2']+'" alt="User Image"></a><a href="#">'+row['client']+'</a></h6>';
				}}, 
				{ "data": "type" },        
                { "data": "device" },
                { "data": "module" },       
				{ "data": "ip" },				
                { "data": "logout-time" }, 
                { 
                    "render": function (data, type, row) {
                        return '<div class="d-flex align-items-center mb-0"><a href="#" class="avatar avatar-xss me-2"><img class="img-fluid rounded-circle" src="' + row['location_image'] + '" alt="User Image"></a>' + row['location_name'] + '</div>';
                    }
                }, 
                { "render": function ( data, type, row ){
					if(row['status'] == "0") { var class_name = "bg-success";var status_name ="Active" }  else { var class_name = "bg-danger";var status_name ="Blocked"}
					return '<span class="badge badge-pill badge-status '+class_name+'" >'+status_name+'</span>';
				}}, 
				{ "render": function ( data, type, row ){
					return '<div class="dropdown table-action"><a href="#" class="action-icon btn btn-xs shadow btn-icon btn-outline-light" data-bs-toggle="dropdown" aria-expanded="false"><i class="ti ti-dots-vertical"></i></a><div class="dropdown-menu dropdown-menu-right"><a class="dropdown-item" href="#"><i class="ti ti-info-triangle text-blue me-1"></i> Block</a></div></div>';
				}},
				
			]
				
		});
	}
});